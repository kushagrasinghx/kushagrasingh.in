...
export default function Experience() {
  ...
  <p className="text-white/60 dark:text-white/60">roles I've held and what I worked on:</p>
        ...
        <div className="mt-10 flex flex-col gap-12">
          {items.map((x, i) => (
            <div key={(x.id || x.title || x.role || 'exp') + i}>
              {x.banner && (
                <div
                  className="w-full max-w-[1000px] aspect-[50/9] bg_gray-800 dark:bg-gray-800 flex items-center justify-center overflow-hidden rounded-md border border-white/10 dark:border-white/10 md:shadow-lg mb-2"
          ...
          <h2 className="font-dmsans text-2xl mb-1 text-white dark:text-white">
                    {x.role || x.title}
                    {x.company ? (
            <span className="text-white/60 dark:text-white/60"> · {x.company}</span>
                    ) : null}
                  </h2>

                  {(x.period || x.location) && (
                    <p className="text-white/50 dark:text-white/50 mb-3">
                      {x.period}
                      {x.period && x.location ? ' • ' : ''}
                      {x.location}
                    </p>
                  ...
                  {x.description && (
                    <p className="text-white/70 dark:text-white/70 mb-4">{x.description}</p>
                  )}

                  {Array.isArray(x.highlights) && x.highlights.length > 0 && (
                    <ul className="list-disc ml-5 text-white/80 dark:text-white/80 space-y-1">
          ...
          {items.length === 0 && (
            <p className="text-white/50 dark:text-white/50">No experience to show (yet).</p>
          )}
        </div>
      </div>
    </main>
