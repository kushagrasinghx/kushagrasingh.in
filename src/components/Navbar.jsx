...
// theme context removed; dark-only site
import { Menu, X } from 'lucide-react'

export default function Navbar() {
  // theme removed; keep unread/menu state only
  ...
  return (
    <nav className='relative z-50 border-b border-white/15 dark:border-white/15'>
      <div className='flex h-[50px] items-center px-4 md:px-[60px]'>
        ...
        >
            {mobileOpen ? <X size={20} className='text-white dark:text-white' /> : <Menu size={20} className='text-white dark:text-white' />}
        </button>
  ...
  <div className='ml-auto w-9 h-9' />
      </div>

      {/* Mobile dropdown */}
      {mobileOpen && (
        <div
          className='md:hidden absolute left-0 top-[50px] w-full border-b border-white/15 dark:border-white/15 backdrop-blur-sm'
